...
function Form_representante({ titulo, id_form, form, fecha, data_campos, isObligatorio = true, parentesco = false }) {
    ...
    // *funcion para desabilitar todos los campos del padre o madre
    const dissable = (e) => {
        // esta parte solo sirve para desativar la opcion de madre y padre en elrepresentatne legal
        const option = document.getElementById(`radio_${id_form}`)
        if (e.target.checked) {
    ...
    const dissableDifunto = (e) => {
        if (e.target.checked) {
            const campos = [`fecha_nacimiento_${id_form}`, `correo_${id_form}`, `telefono_${id_form}`, `telefono_alterno_${id_form}`, `profesion_${id_form}`]
            campos.forEach(element => setValue(`${element}`, ""))
            setDifunto(true)
        } else {
            setDifunto(false)
    ...
    return (
        <div key={`form_${id_form}`} className='d-flex flex-column align-items-center justify-content-around position-relative '>
                ...
                <div className='d-flex mt-2 mt-md-0'>
                    {
                        isObligatorio ?
                            (
                                <div className='d-flex align-items-center ms-2' id={`input_disable_${id_form}`}>
                                    <label htmlFor={`disable_${id_form}`} className='h6 m-0 fw-bold cursor-pointer'>Desabilitar</label>
                                    <input type="checkbox" className='form-check-input ms-1' id={`disable_${id_form}`} name={`disable_${id_form}`} data-bs-toggle="collapse" data-bs-target={`#collapse_${id_form}`} aria-expanded="true" aria-controls={`collapse_${id_form}`}
                                        {
                                        ...register(`disable_${id_form}`, {
                                            onChange: (e) => { dissable(e) }
                                        })
                                        }
                                    />
                                </div>
                        ...
                        isObligatorio ?
                            (
                                <div className='d-flex align-items-center ms-2'>
                                    <label htmlFor={`fallecido_${id_form}`} className='h6 m-0 fw-bold cursor-pointer'>Difunto</label>
                                    <input type="checkbox" className='form-check-input ms-1' id={`fallecido_${id_form}`} name={`fallecido_${id_form}`} disabled={(stateDisable || readOnly)}
                                        {
                                        ...register(`fallecido_${id_form}`, {
                    ...
                    <input type="number" className='d-none'
                        ...
                        ...register(`id_${id_form}`, {
                            required: {
                                value: (!stateDisable),
                                message: "No se ha seleccionado ningun representante"
                            }
                        })
                        }
                    />
            ...
            <Modal_representante id_modal={id_form} id_button={`btn_buscar_${id_form}`} form={form} difuntos={isObligatorio} state={{ setReadOnly, setDifunto, stateDisable, setRepre }} funtion={{ comprobarTipoRepresentante }}
            />
                ...
                <div className='w-100 flex-column flex-md-row d-flex align-items-center justify-content-between my-2'>
...

                    {
                        parentesco ?
                            (
                                <div className='w-40 w-md-100 mt-2 mt-md-0 d-flex justify-content-between mx-1 '>
                                    <div className={`d-flex flex-column w-45 ${(errors[`genero_${id_form}`] && !stateDisable && !readOnly) ? "error" : "bien"}`}>
                                        ...
                                        <select className="form-select input-select " aria-label="Large select example" name={`genero_${id_form}`} id={`genero_${id_form}`} readOnly={readOnly} disabled={(stateDisable || readOnly)}
                                            {
                                    ...
                                    <div className={`d-flex flex-column w-45 ${(errors[`parentesco_${id_form}`] && !stateDisable) ? "error" : "bien"}`}>
                            ...
                            (
                                <div className={`w-40 w-md-100 mt-2 mt-md-0 d-flex flex-column mx-1 ${(errors[`genero_${id_form}`] && !stateDisable && !readOnly) ? "error" : "bien"}`}>
                                    <label className='h6 fw-bold cursor-pointer' htmlFor={`genero_${id_form}`}>Genero</label>
                                    <select className="form-select input-select " aria-label="Large select example" name={`genero_${id_form}`} id={`genero_${id_form}`} defaultValue={null} readOnly={readOnly} disabled={(stateDisable || readOnly)}
                                        {
                                        ...register(`genero_${id_form}`, {
                                            requerid: {
                                                value: !stateDisable,
...
export default Form_representante